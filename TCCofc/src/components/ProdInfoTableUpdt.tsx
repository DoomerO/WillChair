...
import { ProductProps, ProductUpdateProps } from "./code/interfaces";
...
const ProdInfoTableUpdt = ({ ofr_id, update, clear }: prodTableProps) => {
    ...
    async function updateProduct() {
        await axios.put(`${serverUrl}/products/${prod.prod_id}`, {
    ...
    async function updateProductChild() {
        ...
        switch (prod.prod_type) {
            case "Cadeira de Rodas":
                ...
                break;
            case "Bengala":
                await axios.put(`${serverUrl}/products/bengala/${prod.prod_id}`, {
                ...
                break;
            case "Andador":
                await axios.put(`${serverUrl}/products/andador/${prod.prod_id}`, {
                ...
                break;
            case "Muleta":
                await axios.put(`${serverUrl}/products/muleta/${prod.prod_id}`, {
    ...
    const [prodUpdate, setProdUpdate] = useState<ProductUpdateProps>({
        prod_status: "",
        prod_composition: "",
        prod_height: 0,
        prod_weight: 0,
    ...
    return (
        (loading) ? <ComponentLoading width={{base:"100%", md:"80%"}} height="40vh"/> : <TableContainer w={{ base: "100%", md: "80%" }} position="relative">
            <Table variant="unstyled">
                ...
                <Tbody>
                    <Tr bg={colors.bgTableRow1} flexDirection="column" display={{base:"flex", md:"table-row"}}  _dark={{ bg: colors.bgTableRow1_Dark }}>
                        <Td fontWeight="bold" w={{base:"100%", md:"unset"}}>Tipo de Equipamento</Td>
                        <Td w={{base:"100%", md:"unset"}}>{prod.prod_type}</Td>
                    </Tr>
                    <Tr bg={colors.bgTableRow2} flexDirection="column" display={{base:"flex", md:"table-row"}} _dark={{ bg: colors.bgTableRow2_Dark }}>
                        <Td fontWeight="bold">Condição</Td>
                        <Td><Select name='prod_status' onChange={handleSelectChange} value={prodUpdate.prod_status}>
                    ...
                    <Tr bg={colors.bgTableRow2} flexDirection="column" _dark={{ bg: colors.bgTableRow2_Dark }}
                        display={(prod.prod_type == "Cadeira de Rodas" || prod.prod_type == "Andador" || prod.prod_type == "Muleta" || prod.prod_type == "Bengala") ? {base:"flex", md:"table-row"} : "none"}>
                        <Td fontWeight="bold">{prodChildInp.title1}</Td>
                        <Td>{(prod.prod_type != "Cadeira de Rodas") ? <Select name='append1' onChange={handleSelectChange} value={prodUpdate.append1}>
                    ...
                    <Tr bg={colors.bgTableRow1} flexDirection="column" _dark={{ bg: colors.bgTableRow1_Dark }}
                        display={(prod.prod_type == "Cadeira de Rodas" || prod.prod_type == "Andador" && parseInt(prodUpdate.append1) == 1 || prod.prod_type == "Muleta" && parseInt(prodUpdate.append1) == 1 || prod.prod_type == "Bengala" && parseInt(prodUpdate.append1) == 1) ? {base:"flex", md:"table-row"} : "none"}>
                    ...
                    <Tr bg={colors.bgTableRow2} flexDirection="column" _dark={{ bg: colors.bgTableRow2_Dark }}
                        ...
                        <Td>{(prod.prod_type == "Cadeira de Rodas") ? <Select name='append3' onChange={handleSelectChange} value={prodUpdate.append3}>
                    ...
                    <Tr bg={colors.bgTableRow1} flexDirection="column" _dark={{ bg: colors.bgTableRow1_Dark }}
                        ...
                        <Td>{(prod.prod_type == "Bengala") ? <Select name='append4' onChange={handleSelectChange} value={prodUpdate.append4}>
                    ...
                    <Tr bg={colors.bgTableRow2} flexDirection="column" _dark={{ bg: colors.bgTableRow2_Dark }}
                        display={(prod.prod_type == "Andador" || prod.prod_type == "Bengala" || prod.prod_type == "Muleta") ? {base:"flex", md:"table-row"} : "none"}>
                        <Td fontWeight="bold">{prodChildInp.title5}</Td>
                        <Td>{(prod.prod_type == "Muleta") ? <Select name='append5' onChange={handleSelectChange} value={prodUpdate.append5}>
